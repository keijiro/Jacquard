...
Every mark the app carries is the word "Jacquard" set in [Jacquard 12], and each
one is cut from the same source rather than drawn again: the type is a pixel font
on a sixty unit grid, so a glyph reduces to a grid of cells without losing
anything, and `jacquard_grid.py` is that reduction. The glitch — the cap line and
