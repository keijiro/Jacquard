...
public struct FmVoiceState
{
    ...
    public void Trigger(in FmNoteEvent note, float sampleRate)
    {
        ...
        // modulator's is this times its ratio.
        _increment = note.frequency / sampleRate;
    }
    ...
    public float Next(float time)
    {
        ...
        var increment = _increment * _note.PitchScale(time);
        ...
        var amplitude = _note.level * _note.CarrierLevel(time);

        var output = FastMath.Sin(TwoPi * _carrierPhase + mod * index) * amplitude;

        _carrierPhase = Frac(_carrierPhase + increment);
        _modulatorPhase = Frac(_modulatorPhase + increment * _note.modulatorRatio);
    ...
    float _increment;       // Carrier phase per sample, before the pitch envelope
